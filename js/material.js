...
class TensionWeb {
    ...
    createNodes() {
        ...
        // Corner anchors
        this.nodes.push({ x: 0, y: 0, anchor: true });
        this.nodes.push({ x: w, y: 0, anchor: true });
        this.nodes.push({ x: 0, y: h, anchor: true });
        this.nodes.push({ x: w, y: h, anchor: true });
        
        // Edge anchors
        this.nodes.push({ x: w * 0.3, y: 0, anchor: true });
        this.nodes.push({ x: w * 0.7, y: 0, anchor: true });
        this.nodes.push({ x: 0, y: h * 0.4, anchor: true });
        this.nodes.push({ x: w, y: h * 0.6, anchor: true });
        this.nodes.push({ x: w * 0.2, y: h, anchor: true });
        this.nodes.push({ x: w * 0.8, y: h, anchor: true });
        
        // Floating nodes (react to mouse)
        for (let i = 0; i < 4; i++) {
            this.nodes.push({
                x: Math.random() * w * 0.6 + w * 0.2,
                y: Math.random() * h * 0.6 + h * 0.2,
                anchor: false,
                baseX: Math.random() * w * 0.6 + w * 0.2,
    ...
    animate() {
        ...
        // Draw tension lines
        this.ctx.strokeStyle = '#c8b4a0';
        this.ctx.lineWidth = 0.5;
        
        // Connect each floating node to multiple anchors
        const anchors = this.nodes.filter(n => n.anchor);
        const floaters = this.nodes.filter(n => !n.anchor);
        ...
        floaters.forEach(floater => {
            anchors.forEach(anchor => {
                ...
                // Curved line through floater
                const midX = (anchor.x + floater.x) / 2;
                const midY = (anchor.y + floater.y) / 2;
                const offset = 20;
        ...
        // Connect floaters to each other
        for (let i = 0; i < floaters.length; i++) {
            for (let j = i + 1; j < floaters.length; j++) {
                this.ctx.beginPath();
                this.ctx.moveTo(floaters[i].x, floaters[i].y);
                this.ctx.lineTo(floaters[j].x, floaters[j].y);
                this.ctx.stroke();
            }
        }
